...
function dispatchAction(action, payload) {
  ...
  if (action === 'generateInspectionReport') return generateInspectionReportAI(payload);
...
function generateInspectionReportAI(payload) {
  const settings = getSystemSettings();
  if (!settings.geminiKey || !settings.geminiKey.trim()) {
    return { success: false, message: 'ยังไม่ได้ตั้งค่า Gemini API Key จึงไม่สามารถสร้างรายงานด้วย AI ได้' };
  }
  const prompt = String(payload && payload.prompt || '').trim();
  if (!prompt) return { success: false, message: 'ไม่พบข้อมูลสำหรับจัดทำรายงาน' };
  if (prompt.length > 30000) return { success: false, message: 'ข้อมูลรายงานยาวเกินกำหนด กรุณาเลือกช่วงข้อมูลให้น้อยลง' };
  try {
    const reply = callGeminiAPI(prompt, '', settings);
    if (!reply || /^AI แจ้งข้อผิดพลาด|^ขออภัยค่ะ ขณะนี้น้องศึกษาไม่สามารถดึงข้อมูลได้/.test(reply)) {
      return { success: false, message: reply || 'AI ไม่สามารถสร้างรายงานได้ในขณะนี้' };
    }
...
function offlineChatReply(msg) {
  if (msg.includes('สวัสดี') || msg.includes('hello') || msg.includes('hi') || msg.includes('เป็นใคร') || msg.includes('อะไร') || msg.includes('ชื่ออะไร') || msg.length <= 10) {
    return 'อัสลามุอะลัยกุม! ขอสันติจงมีแด่ท่าน ฉันคือ "น้องศึกษา" ผู้ช่วย AI ยินดีให้คำปรึกษาเกี่ยวกับระบบนิเทศออนไลน์สถาบันศึกษาปอเนาะ จังหวัดนราธิวาสครับ<br>สอบถามเรื่องชุดเครื่องมือนิเทศ 8 ฉบับ เกณฑ์การให้คะแนน วงรอบการนิเทศ หรือข้อมูลสถาบันได้เลยครับ';
  }
  if (msg.includes('เกณฑ์') || msg.includes('คะแนน')) {
    return 'ชุดเครื่องมือนิเทศปอเนาะมี 8 ฉบับครับ:<br>• แบบที่ 1 สำรวจข้อมูลพื้นฐานและความพร้อมในการใช้หลักสูตร (ไม่ให้คะแนน)<br>• แบบที่ 2 การบริหารและการใช้หลักสูตร (8 องค์ประกอบ ให้ 4/3/2/1 หรือ N/A รวม 32)<br>• แบบที่ 3 วิเคราะห์หน่วยและแผนการจัดการเรียนรู้ (เชิงคุณภาพ)<br>• แบบที่ 4 สังเกตการจัดการเรียนรู้ในชั้นเรียน (9 ด้าน ให้ 0-3 รวม 27)<br>• แบบที่ 5 ติดตามการวัด ประเมินผล และพัฒนาการผู้เรียน (เชิงคุณภาพ)<br>• แบบที่ 6 สะท้อนผลและแผนพัฒนาหลังการนิเทศ AAR + Action Plan<br>• แบบที่ 7 นิเทศติดตามสำหรับผู้ที่ไม่ใช่ศึกษานิเทศก์ (12 ข้อ ให้ 0/1/2 รวม 24)<br>• แบบที่ 8 บันทึกภาคสนามสำหรับผู้ไม่ใช่ศึกษานิเทศก์ (12 ข้อด้านการนำหลักสูตร/การเรียนรู้ และ 4 ข้อสังเกตผลผู้เรียน ให้ 2/1/0/N/A รวมไม่เกิน 32)';
  }
  ...
  if (msg.includes('แบบที่ 8')) {
    return 'แบบที่ 8 เป็นบันทึกภาคสนามสำหรับผู้ที่ไม่ใช่ศึกษานิเทศก์ มี 12 ข้อด้านการนำหลักสูตรและการจัดการเรียนรู้ พร้อม 4 ข้อสังเกตผลการเรียนรู้ของผู้เรียน ให้คะแนน 2/1/0 หรือ N/A เพื่อช่วยเหลือและพัฒนา ไม่ใช่จับผิดหรือจัดอันดับครับ';
  }
  ...
  return 'สวัสดีครับ ผมน้องศึกษา พร้อมให้คำแนะนำเรื่องชุดเครื่องมือนิเทศปอเนาะ 8 ฉบับ วงรอบการนิเทศ และเกณฑ์การประเมินได้เลยครับ';
}
...
function callGeminiAPI(userMessage, contextData, settings) {
  ...
  const systemPrompt = 'คุณคือ "น้องศึกษา" ผู้ช่วยอัจฉริยะ AI บุคลิกสุภาพ เป็นมิตร กระตือรือร้น ให้เกียรติผู้ใช้งาน ใช้ภาษาไทยที่ถูกต้อง เป็นทางการแต่นุ่มนวล และลงท้ายประโยคด้วย "ครับ/ค่ะ" เสมอ ตอบให้กระชับ ตรงประเด็น ใช้ Bullet points จัดรูปแบบให้อ่านง่าย กฎสำคัญ: เมื่อใดก็ตามที่ผู้ใช้ทักทาย (เช่น "สวัสดี", "hello", "hi") หรือเป็นการสนทนาเริ่มต้น ให้เริ่มคำตอบด้วย "อัสลามุอะลัยกุม! ขอสันติจงมีแด่ท่าน" เสมอ ตามด้วยแนะนำตัว "ฉันคือ \'น้องศึกษา\' ผู้ช่วย AI ของระบบนิเทศออนไลน์สถาบันศึกษาปอเนาะ จังหวัดนราธิวาส" และเสนอความช่วยเหลือ\n\n' +
    ...
    "4. ระบบนิเทศออนไลน์สถาบันศึกษาปอเนาะ จังหวัดนราธิวาส (ชุดเครื่องมือนิเทศ 8 ฉบับ):\n" +
    ...
    "- แบบที่ 8 บันทึกภาคสนามสำหรับผู้ไม่ใช่ศึกษานิเทศก์ (12 ข้อด้านการนำหลักสูตร/การเรียนรู้ และ 4 ข้อสังเกตผลผู้เรียน ให้ 2/1/0/N/A รวมไม่เกิน 32)\n" +
